...
def signup(request):
    if request.method == 'POST':
        ...
        return render(request, 'verify_otp.html', {'email': email, 'otp': str(verification_code)})
    ...
    if request.method == 'POST':
        if uotp == otp:
            try:
                ...
                user = User.objesignupcts.create(
                        email=email,
                        pswd=otp,       
                        )
                user_pk = user.pk
...
def resend_code(request):
    if request.method == 'POST':
        ...
        return JsonResponse({'message': 'Verification code resent.', 'new_otp': verification_code})
...
def work_space(request, pk):
    ...
    Eden_AI = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoiYjc3OTBkZjQtMzhiMy00NTQ0LThjMzAtNWU1ZmIyMzg4NjgyIiwidHlwZSI6ImFwaV90b2tlbiJ9.niTfYcb-25kkmS7Z9y6pqBCw01pMhZyRZJrlx2mSt8o"
    ...
    if request.method == 'POST':
        ...
        if selected_option == "explainer":
            generated_text = generate_text(user_input_text, Eden_AI)
